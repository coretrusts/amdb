#!/bin/bash
# -*- coding: utf-8 -*-
# 简化版GitHub部署脚本（使用Git，不依赖GitHub CLI）
...
echo "=== AmDb GitHub部署脚本（简化版）==="
echo ""
echo "此脚本将创建三个仓库："
echo "  1. $ORG/$REPO_SOURCE - 源代码"
echo "  2. $ORG/$REPO_DOCS - 文档"
echo "  3. $ORG/$REPO_RELEASES - 发行版"
echo ""
...
init_and_push() {
    ...
    echo "✓ Git仓库初始化完成"
    echo ""
    echo "⚠️  请先在GitHub上创建仓库: https://github.com/organizations/$ORG/repositories/new"
    echo "   仓库名: $repo_name"
    echo "   描述: $description"
    echo "   可见性: Public"
    echo ""
    ...
    if git diff --staged --quiet; then
        ...
        echo ""
        echo "准备推送到: https://github.com/$ORG/$repo_name.git"
        echo "如果推送失败，请检查："
        echo "  1. 仓库是否已创建"
        echo "  2. 是否有推送权限"
        echo "  3. 是否需要设置认证"
...
# 复制脚本文件
for file in amdb-cli amdb_manager.py create_*.py verify_*.py; do
    if [ -f "$file" ]; then
        cp "$file" "$TEMP_SOURCE/" 2>/dev/null || true
    fi
done

init_and_push "$REPO_SOURCE" "AmDb - 区块链优化数据库系统（源代码）" "$TEMP_SOURCE"
rm -rf "$TEMP_SOURCE"
...
init_and_push "$REPO_DOCS" "AmDb - 文档和API参考" "$TEMP_DOCS"
rm -rf "$TEMP_DOCS"
...
init_and_push "$REPO_RELEASES" "AmDb - 发行版和可执行文件" "$TEMP_RELEASES"
rm -rf "$TEMP_RELEASES"
